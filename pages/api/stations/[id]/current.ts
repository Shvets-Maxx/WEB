...
import type { ApiResponse, AirQualitySnapshot } from '@/types';
...
function handler(
  req: NextApiRequest,
  res: NextApiResponse<ApiResponse<AirQualitySnapshot>>,
): void {
  if (req.method !== 'GET') {
  ...
  sendSuccess(res, latestSnapshot(station).latest);
}

export default withApiLogger(handler, 'GET /api/stations/[id]/current');
